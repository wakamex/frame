  ...
  chains: [
    {
      ...
      js: `(() => {
        ...
        const ethBtn = buttons.find(b => b.textContent.includes('Ethereum'));
        if (ethBtn) { ethBtn.click(); return 'clicked: ' + ethBtn.textContent.substring(0, 40); }
        return 'no Ethereum button found, buttons: ' + buttons.length;
      })()`
    }
  ]
}
